export default {
  ...
  admin: {
    ...
    dataAnalysis: {
      title: '数据分析',
      description: '按账号和分组分析响应时间、首字时长与错误率',
      dimension: '分析维度',
      byAccount: '按账号',
      byGroup: '按分组',
      timeRange: '时间范围',
      ...
      allAccounts: '全部账号',
      allGroups: '全部分组',
      platformPlaceholder: 'openai / claude',
    ...
    accounts: {
      ...
      vertexLabel: 'Vertex',
      vertexDesc: '服务账号',
      vertexAnthropicHint: '使用 Google Cloud 服务账号 JSON 通过 Vertex AI 调用 Anthropic Claude。建议配置模型映射，将客户端 Claude 模型名映射到 Vertex 模型 ID。',
      vertexGeminiHint: '使用 Google Cloud 服务账号 JSON 访问 Vertex AI Gemini。建议将 Vertex 账号放入独立分组，避免和 AI Studio/Gemini OAuth 同模型混调。',
      vertexSaJsonLabel: '服务账号 JSON',
      vertexSaJsonLoaded: '已读取服务账号 JSON',
      vertexSaJsonDrop: '拖入服务账号 JSON',
      ...
      vertexSaJsonEditHint: '服务账号 JSON 不在编辑页显示；需要更换 JSON 时请删除账号后重新创建。',
      vertexProjectIdLabel: '项目 ID',
      vertexClientEmailLabel: '客户端邮箱',
      vertexLocationLabel: '区域',
      vertexProjectIdPlaceholder: '从 JSON 自动读取',
      vertexLocationHint: '不同 Vertex 模型可用区域可能不同，这里选择账号默认端点区域。',
      vertexLocationRequired: '请填写 Vertex 区域',
      vertexSaJsonMissingFields: '服务账号 JSON 缺少 project_id、client_email 或 private_key',
      vertexSaJsonMissingProjectId: '服务账号 JSON 缺少 project_id',
      vertexSaJsonMissingClientEmail: '服务账号 JSON 缺少 client_email',
      vertexSaJsonInvalid: '服务账号 JSON 格式无效',
      vertexSaJsonRequired: '请上传服务账号 JSON',
      ...
      oauth: {
        ...
        gemini: {
          ...
          redirectUriHint: '需要在 Google OAuth 客户端中配置，且必须与此处完全一致。',
          confirmRedirectUri: '我已在 Google OAuth 客户端中配置了该 Redirect URI（必须完全一致）',
          invalidRedirectUri: 'Redirect URI 必须是合法的 http(s) URL',
          redirectUriNotConfirmed: '请确认 Redirect URI 已在 Google OAuth 客户端中正确配置',
          ...
          missingProjectId:
            'GCP 项目 ID 获取失败：您的 Google 账号未关联有效的 GCP 项目。请前往 Google Cloud Console 激活 GCP 并绑定信用卡，或在授权时手动填写项目 ID。',
          ...
          needsProjectIdDesc: '需要 GCP 项目与项目 ID',
          ...
          aiStudioNotConfiguredTip:
            'AI Studio OAuth 未配置：请先设置 GEMINI_OAUTH_CLIENT_ID / GEMINI_OAUTH_CLIENT_SECRET，并在 Google OAuth 客户端添加 Redirect URI：http://localhost:1455/auth/callback（Consent Screen scopes 需包含 https://www.googleapis.com/auth/generative-language.retriever）',
          aiStudioNotConfigured:
            'AI Studio OAuth 未配置：请先设置 GEMINI_OAUTH_CLIENT_ID / GEMINI_OAUTH_CLIENT_SECRET，并在 Google OAuth 客户端添加 Redirect URI：http://localhost:1455/auth/callback'
        },
      ...
      gemini: {
        ...
        oauthType: {
          ...
          builtInRequirement: '需要 GCP 项目并填写项目 ID。',
          gcpProjectLink: '创建项目',
          customTitle: '自定义授权（AI Studio OAuth）',
          customDesc: '使用管理员预设的 OAuth 客户端，适合组织管理。',
          customRequirement: '需管理员配置客户端 ID 并加入测试用户白名单。',
          badges: {
            recommended: '推荐',
            highConcurrency: '高并发',
            noAdmin: '无需管理员配置',
        ...
        setupGuide: {
          ...
          activationItems: {
            ...
            gcpProject: '激活 GCP 项目，获取 Code Assist 所需项目 ID。'
          },
          links: {
            countryCheck: '检查归属地',
    ...
    settings: {
      ...
      soraS3: {
        ...
        gdrive: {
          ...
          oauthFieldsRequired: '请先填写客户端 ID 和客户端密钥',
